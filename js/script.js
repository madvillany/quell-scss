...
//jQuery(document).ready(
	...
	{
		$(".mobilebkg").removeAttr("style");
	}
	
	//fittext
	//jQuery(".fittext").fitText();
	
//removed tabs for funds	
// //tabs
...
// });
	...
	$(".funds .fund").on("click",function(){
		$(".fund").addClass("off").removeClass("on");
		$(this).removeClass("off").addClass("on");
		
		var text = $(this).find("p").text();
		$(".fundcontent").text(text);
		$(".fundcontent").css("visibility","visible");
	});
   ...
   $(".desktop.navbar").addClass("at-top");
	$(window).scroll(function() {
		...
		if (($(".desktop.navbar").offset().top > 5) && $(window).width() > mobilewidth) {
			...
			$(".desktop.navbar").removeClass("at-top");
		} else {
	...
	$(window).scroll(function(){
	 ...
	 //hide take action if take action is in view
		if(($(this).scrollTop() > 1000  ) && ($(window).width() > mobilewidth))
		{
